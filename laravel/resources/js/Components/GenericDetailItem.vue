  ...
  methods: {
    ...
    optionsFor(column) {
      if (typeof column.options === 'function') {
        return column.options(this.row, this.optionSources) || [];
      }
      if (column.optionsSource === 'row') {
        return this.row?.[column.optionsKey] || [];
      }
      if (Array.isArray(column.options)) {
        return column.options;
      }
      return this.optionSources?.[column.optionsKey] || [];
    },
    onChange(column, { value, option } = {}) {
      const resolvedValue = value || (option ? option[column.valueField || 'id'] : null);
      this.row[column.model] = resolvedValue;
  ...
  <tr :data-detail-temp-uuid="rowId()">
    ...
    <template v-for="column in (columns || [])" :key="column.key">
      <SelectableCell
        ...
        :portal-target="column.portalTarget || 'body'"
        @move-to-cell="handleNavigate"
        @change-option="(option) => onChange(column, { option })"
