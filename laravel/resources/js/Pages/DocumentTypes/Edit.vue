...
export default {
    ...
    methods: {
        ...
        hydrateForm() {
            this.form = {
                prefix: this.documentTypeEdit.prefix ?? '',
                name: this.documentTypeEdit.name ?? '',
                module: this.documentTypeEdit.module ?? '',
                affectsInventory: Boolean(this.documentTypeEdit.affectsInventory),
                inventoryMovementType: this.documentTypeEdit.inventoryMovementType ?? '',
                allowNegativeInventory: Boolean(this.documentTypeEdit.allowNegativeInventory),
                status: Boolean(this.documentTypeEdit.status),
                hasPrefix: Boolean(this.documentTypeEdit.hasPrefix),
                hasDate: Boolean(this.documentTypeEdit.hasDate),
                dateFormat: this.documentTypeEdit.dateFormat ?? '',
                lengthSequence: this.documentTypeEdit.lengthSequence ?? '',
            };
        },
        async submitForm() {
            this.clientErrors = {};
            ...
            try {
                ...
                const confirmed = await showConfirm(
                    'warning',
                    '¡Alerta!',
                    '¿Está seguro que desea actualizar el tipo documento?',
                    'Sí, actualizar'
                );
                if (!confirmed) {
                    this.loading = false;
                ...
                const response = await fetchPetition(`/document-types/${this.uuid}`, {
                    method: 'PUT',
                    body: this.form,
                });
                if (response.ok) {
                    showAlert('success', '¡Éxito!', 'Tipo documento actualizado correctamente', 1500);
                    router.visit('/tipos-documentos');
                } else {
                    const data = response.data;
                    const message = data?.message || (data?.errors ? Object.values(data.errors || {}).flat().join(' ') : 'Ocurrió un error al actualizar el tipo documento');
                    showAlert('error', 'Error', message, 3000);
                }
            } catch (error) {
                showAlert('error', 'Error inesperado', 'Ocurrió un error al actualizar el tipo documento', 3000);
            } finally {
                this.loading = false;
            }
        }
...
<template>
    <Layout>
        <PageHeader title="Editar Tipo Documento" pageTitle="Tipos Documentos" />
        <div class="row">
            <div class="col-8">
                <div class="card">
                    <div class="card-header">
                        <div class="form-section-title">
                            ...
                            <p class="text-muted mb-0">Actualiza los campos del tipo documento.</p>
                        </div>
                    </div>
                    <div class="card-body">
                        <form @submit.prevent="submitForm" ref="formRef">
                            <div class="row g-3">
                                <div class="col-12 col-md-6 mb-3" data-form-error-anchor="prefix">
                                    ...
                                    <small v-if="prefixExistsError" class="text-danger"><i class="ri-error-warning-line mx-1"></i>{{ prefixExistsError }}</small>
                                </div>
